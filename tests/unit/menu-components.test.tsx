...
describe('menu components', () => {
  ...
  it('aciona handlers e normaliza campos do passo de endereço', async () => {
    ...
    inputs[0].props.onChange({ target: { value: 'Minha Casa' } })
    inputs[1].props.onChange({ target: { value: '01001-000' } })
    inputs[2].props.onChange({ target: { value: 'Praça da Sé' } })
    inputs[3].props.onChange({ target: { value: '10' } })
    inputs[4].props.onChange({ target: { value: 'Apto 2' } })
    inputs[5].props.onChange({ target: { value: 'Centro' } })
    inputs[6].props.onChange({ target: { value: 'São Paulo' } })
    inputs[7].props.onChange({ target: { value: 'sp' } })
    buttons[0].props.onClick()
    buttons[1].props.onClick()

    expect(onAddressChange).toHaveBeenCalledTimes(8)
    expect(onCepLookup).toHaveBeenCalledWith('01001000')
    expect(onAddressChange.mock.calls[0][0]({})).toEqual({ label: 'Minha Casa' })
    expect(onAddressChange.mock.calls[1][0]({})).toEqual({ zip_code: '01001000' })
    expect(onAddressChange.mock.calls[2][0]({})).toEqual({ street: 'Praça da Sé' })
    expect(onAddressChange.mock.calls[3][0]({})).toEqual({ number: '10' })
    expect(onAddressChange.mock.calls[4][0]({})).toEqual({ complement: 'Apto 2' })
    expect(onAddressChange.mock.calls[5][0]({})).toEqual({ neighborhood: 'Centro' })
    expect(onAddressChange.mock.calls[6][0]({})).toEqual({ city: 'São Paulo' })
    expect(onAddressChange.mock.calls[7][0]({})).toEqual({ state: 'SP' })
  ...
  it('não dispara busca de CEP enquanto o valor ainda está incompleto', async () => {
    ...
    const inputs = elements.filter((element) => element.type === 'input')
    inputs[1].props.onChange({ target: { value: '01001-00' } })
